describe("Accepted Input types", function() {
	...
	beforeEach(function() {
    	coinParse = new CoinParse();
  	});

	it("accepts single digit",function(){
 		var input = '4';
 		expect(coinParse.toPennies(input)).toEqual(4);
	});

	it("accepts double digit",function(){
 		var input = '85';
 		expect(coinParse.toPennies(input)).toEqual(85);
	});

	it("accepts pence symbol",function(){
 		var input = '197p';
 		expect(coinParse.toPennies(input)).toEqual(197);
	});
	
	it("accepts pence symbol single digit",function(){
 		var input = '2p';
 		expect(coinParse.toPennies(input)).toEqual(2);
	});

	it("accepts pounds decimal",function(){
 		var input = '1.87';
 		expect(coinParse.toPennies(input)).toEqual(187);
	});

	it("accepts pound symbol",function(){
 		var input = '£1.23';
 		expect(coinParse.toPennies(input)).toEqual(123);
	});

	it("accepts single digit pound symbol",function(){
 		var input = '£2';
 		expect(coinParse.toPennies(input)).toEqual(200);
	});

	it("accepts double digit pound symbol",function(){
 		var input = '£10';
 		expect(coinParse.toPennies(input)).toEqual(1000);
	});
	...
	it("accepts missing pence",function(){
 		var input = '£1p';
 		expect(coinParse.toPennies(input)).toEqual(100);
	});
	...
	it("accepts buffered zeros",function(){
 		var input = '001.41p';
 		expect(coinParse.toPennies(input)).toEqual(141);
	});

	it("accepts rounding three decimal places to two",function(){
 		var input = '4.235p';
 		expect(coinParse.toPennies(input)).toEqual(424);
	});

	it("accepts rounding with symbols",function(){
 		var input = '£1.257422457p';
 		expect(coinParse.toPennies(input)).toEqual(126);
	});
});
...
describe("Invalid Input types", function() {
	...
	it("Should not accept empty string",function(){
 		var input = '';
 		expect(coinParse.toPennies(input)).toEqual(0);
	});
